<template lang="html">
  <div class="m-colorPicker" tabindex="-1" @blur="closePanel()" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    <!-- 颜色显示小方块 -->
    <div class="colorBtn" ref="p"
      v-bind:style="`background-color: ${showColor}; width: ${width}; height: ${height}`"
    ...
    <!-- 颜色色盘 -->
    <div class="box" v-bind:class="{ open: openStatus }" :style="{top, right, bottom}">
      <div class="hd">
...
.m-colorPicker{
  ...
  .box{
    position: fixed; width: 190px; background: #fff; border: 1px solid #ddd; visibility: hidden; border-radius: 2px; margin-top: 2px; padding: 10px; padding-bottom: 5px; box-shadow: 0 0 5px rgba(0,0,0,.15); opacity: 0;;
