<template>
	<view class="">
		<view class="text-normal" @click="open">
			<text class="color-tip" v-if="!currentMsg">{{(areas && areas.length) ? msg : '加载中...'}}</text>
			<text v-else>{{currentMsg}}</text>
		</view>
		<u-popup :show="show" @open="open" @close="close" class="x-picker-ctx" type="bottom">
			<view class="x-picker-hd u-flex-row" @touchmove.stop.prevent catchtouchmove="true">
			  <view class="x-picker-btn" @tap="close">取消</view>
