<template>
	<!-- 优惠券面板 -->
	<view class="mask u-flex-row" :class="couponVisible===0 ? 'none' : couponVisible===1 ? 'show' : ''" @click="toggleMask">
		<view class="mask-content u-block">
			<view class="coupon-item" v-for="(item,index) in list" :key="index">
				...
				<view class="op u-flex-row">
					...
					<text class="op-btn" v-if="current != index" @click.stop="useCoupon(item, index)">使用</text>
					<text class="op-btn btn-gray" v-else @click.top="cancel(item, index)">取消</text>
				</view>
			</view>
		</view>
	</view>
...
<script>
	export default {
		...
		methods: {
			...
			cancel(item, index) {
				this.current = -1;
				this.$emit('cancel', item.coupon, index);
			}
		}
	}
</script>
