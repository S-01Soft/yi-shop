<template>
	<view>
		...
		<navigator :url="addressUrl" class="address-section">
			<view class="order-content u-flex-row" v-if="currentAddress.contactor_name">
		...
		<view class="goods-section">
			<!-- 商品列表 -->
			<view class="g-item u-flex-row" v-for="(item, index) in products" :key="index">
				<image :src="item.product.image[0] && $tools.fixurl(item.product.image[0])"></image>
				<view class="right">
		...
		<view class="yt-list" v-if="userinfo.score > 0 && totalPrice.score_price > 0">
		...
		<view class="yt-list" v-if="userinfo.money > 0">
			<view class="yt-list-cell b-b">
		...
		<view class="yt-list">
			...
			<view class="yt-list-cell b-b" v-if="totalPrice.discount_price">
			...
			<view class="yt-list-cell b-b total">
				...
				<text class="cell-tip">￥ {{order_total_price}}</text>
			</view>
		</view>
		
		<!-- 底部 -->
		<view class="footer u-flex-row">
			<view class="price-content u-flex-row">
				<text>总金额</text>
		...
		<coupon :list="couponList" v-model="couponVisible" @cancel="couponCancel" @change="couponChange"></coupon>
	...
	export default {
		...
		async onLoad(options){
			...
			if (options.sku_id) {
				...
				let products = [];
				let list = await this.getCartProducts();
				list.forEach(item => {
					if (item.is_selected) products.push(item);
				});
			...
			if (this.$tools.has_addon('shopcoupon')) this.getUsableCoupons()
		},
		computed: {
			...mapState({
			...
			addressUrl() {
				let form = Object.assign(this.options, {source: 1})
				return '/pages/address/index?' + this.$tools.queryStringify(form)
			}
		},
		methods: {
			...mapActions(['getCartProducts', 'getUserAddress', 'getUserinfo']),
			...
			stopPrevent(){},
			getUsableCoupons() {
				this.$http.post('shopcoupon/api/coupon/usableCoupon', this.form).then(data => {
			...
			couponCancel(item, index) {
				this.currentCoupon = {};
				this.form.coupon_id = 0;
				this.couponVisible = 0;
				this.getTotalPrice();
			},
			useScoreHandle(val) {
				this.form.use_score = val;
				this.getTotalPrice();
			},
			useMoneyHandle(val) {
				this.form.use_money = val;
				this.getTotalPrice();
			}
		}
	}
